...
class GenerationMixin:
    ...
    def generate(
        ...
        negative_prompt_ids: Optional[torch.Tensor] = None,
        negative_prompt_attention_mask: Optional[torch.Tensor] = None,
        classifier: Optional["PreTrainedModel"] = None,
        loaded_model_name: str = "1",
        **kwargs,
    ...
    def _dola_decoding(
        ...
        classifier: Optional["PreTrainedModel"],
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # loaded_model = "1"
            # if loaded_model_name!="1" and "bcq" in loaded_model_name:
            #     input_dim = 90
            #     action_dim = 2  # binary classification: 0 or 1
            #     device = 'cpu'
            #     loaded_model = BCQAgent(input_dim, action_dim, hidden_layers=[1024, 512, 256],
            #               gamma=1.0, lr=1e-4, device='cpu', bc_threshold=0.3)
            #     checkpoint = torch.load(loaded_model_name, map_location=device)
            ...
            if dola_act>100:
                classifier = None
            next_token_logits,tmp,next_ = _dola_select_contrast(
                candidate_premature_layers, candidate_premature_logits, final_logits, classifier, loaded_model_name, input_ids, logits_processor
            )
...
def _dola_select_contrast(
    ...
    tokenizer = 1
    if classifier or loaded_model_name=='1' or 'hxx' in loaded_model_name:
        tokenizer = 1
    else:
        tokenizer = AutoTokenizer.from_pretrained(loaded_model_name,trust_remote_code=True)
    first_row, premature_idx = [],[]
    for idx, layer_id in enumerate(candidate_premature_layers):
        ...
        for batch_idx in range(logits.shape[0]):
            if tokenizer!=1:
                print(f"Premature Layer {layer_id},{idx}, Sequence {batch_idx}:")
            for token_prob, token_id in zip(topk.values[batch_idx], topk.indices[batch_idx]):
                ...
                if classifier:
                    first_row.append(token_id.item())
                    premature_idx.append(token_id.item())
                    first_row.append(token_prob.item())
            # print("\n")
    ...
    flag,in_premature,new_predictions=0,0,1
    for batch_idx in range(softmax_mature_layer.shape[0]):
        if tokenizer!=1:
            print(f"Mature Layer:")
    ...
    if classifier:
        ...
        if "bcq" in loaded_model_name:
            state = first_row.values.astype(np.float32).flatten()
            new_predictions = [classifier.BC_predict(state)]
        else:
            new_predictions = classifier.predict(first_row)
    ...
    # if flag or in_premature:
    #     print("Mature Layer Reduce premature_layer:",premature_layer,js_divs)
    # else:
    #     print("Mature Layer Did Not Reduce premature_layer:",premature_layer,js_divs)

    base_logits = candidate_premature_logits[premature_layer]
    final_logits, base_logits = _relative_top_filter(final_logits, base_logits)
    logits = final_logits - base_logits

    next_token_logits = logits.to(input_ids.device)
    next_token_scores = logits_processor(input_ids, next_token_logits)
    next_token_ids = torch.argmax(next_token_scores, dim=-1)
    next_token_id = next_token_ids[0].item()
    if next_token_id in premature_idx:
        in_premature=1
    
    # print("Nexttokens:",next_token_id)
    # If mature layer token not in premature layer, it should follow the mature layer
    # if not in_premature:
    #     print("NOT in premature")
    #     return logits,0,next_token_id
    
    # Not do dola
    if classifier and 0 in new_predictions:
        # print("NO DoLa")
        return logits,1,next_token_id
    return logits,0,next_token_id
